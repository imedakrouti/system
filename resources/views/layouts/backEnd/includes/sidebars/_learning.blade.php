      ...
      <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
        ...
        <li class=" nav-item {{request()->segment(2)=='settings'?'active':''}}"><a href="#"><i class="la la-gears"></i><span class="menu-title" data-i18n="nav.dash.main">{{ trans('admin.settings') }}</span></a>
            <ul class="menu-content">                                                 
                ...
                <li class="{{request()->segment(3)=='division-subjects'?'active':''}}"><a class="menu-item" href="{{route('division-subjects.index')}}" ><i class="la la-angle-{{session('lang') =='ar' ?'left':'right'}}"></i> {{ trans('learning::local.division_subjects') }}</a></li>                         
                <li class="{{request()->segment(3)=='grade-subjects'?'active':''}}"><a class="menu-item" href="{{route('grade-subjects.index')}}" ><i class="la la-angle-{{session('lang') =='ar' ?'left':'right'}}"></i> {{ trans('learning::local.grade_subjects') }}</a></li>                         
