                    ...
                    @foreach ($questions as $question)
                        <div class="bs-callout-info callout-border-left callout-square callout-bordered callout-transparent mt-1 p-1">
                            <strong class="black">{{$n}} - {{$question->question_type}} <span class="red">{{ trans('learning::local.mark') }} 
                                {{$question->mark}}</span> | {{ trans('student.your_mark') }} 
                                {{-- student mark in question --}}
                                @foreach ($question->userAnswers as $user_answer)
                                    @if ($question->id == $user_answer->question_id)
                                        <span class="blue">{{$user_answer->mark}}</span>                                         
                                    @endif
                                @endforeach
                            </strong>
                            ...
                            @if ($question->question_type == trans('learning::local.question_essay') ||
                                ...
                                <div class="form-group mt-1">
                                    <label><strong>{{ trans('learning::local.correct_answer') }} </strong></label>
                                    @foreach ($question->answers as $answer)
                                        @if ($answer->question_id == $question->id)
                                            <textarea style="font-size: 18px" class="form-control" cols="30" rows="10" >{!!$answer->answer_text!!}</textarea>                                                                                    
                                        @endif    
                                    @endforeach
                                </div>
                            @endif                        
