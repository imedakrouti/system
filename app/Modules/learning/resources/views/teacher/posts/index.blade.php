                ...
                <div class="card-content">
                    <div class="card-body">
                        ...
                        <div class="btn-group mr-1 mb-1">
                            <button type="button" class="btn btn-info btn-min-width dropdown-toggle round"
                                data-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">{{ trans('learning::local.class_work') }}</button>
                            <div class="dropdown-menu">
                                <a class="dropdown-item" onclick="assignment()" href="#"><i
                                        class="la la-sticky-note"></i>{{ trans('learning::local.assignment') }}</a>
                                <a class="dropdown-item" onclick="question()" href="#"><i
                                        class="la la-question"></i>{{ trans('learning::local.add_questions') }}</a>
                            </div>
                        </div>
                        {{-- end homework --}}
                        ...
                        <ul>
                            ...
                            @foreach ($exams as $exam)
                                <li><a target="_blank"
                                        href="{{ route('teacher.preview-exam', $exam->id) }}">{{ $exam->exam_name }}</a>
                                </li>
                            @endforeach
                        </ul>
                        <hr>
                        <h4 class="card-title" id="heading-icon-dropdown">
                            <strong>{{ trans('learning::local.classrooms') }}</strong></h4>
                        <ol>
                            @foreach (employeeClassrooms() as $class_item)
                                @if ($class_item->id == $classroom->id)
                                    <li>{{ session('lang') == 'ar' ? $class_item->ar_name_classroom : $class_item->en_name_classroom }}
                                    </li>
                                @else
                                    <li><a
                                            href="{{ route('posts.index', $class_item->id) }}">{{ session('lang') == 'ar' ? $class_item->ar_name_classroom : $class_item->en_name_classroom }}</a>
                                    </li>
                                @endif
                            @endforeach
                        </ol>
                        ...
                        <div class="comment-form">
                            <span class="avatar avatar-online">
                                @isset(authInfo()->image_profile)
                                    <img style="width: 50px;height:50px;max-width: 50px;float:right"
                                        src="{{ asset('images/imagesProfile/' . authInfo()->image_profile) }}" alt="avatar">
                                @endisset
                                @empty(authInfo()->image_profile)
                                    <img style="width: 50px;height:50px;max-width: 50px;float:right"
                                        src="{{ asset('images/website/male.png') }}" alt="avatar">
                                @endempty
                            </span>
