                ...
                <div class="card-content collapse show">
                    <div class="card-body">
                        <h3 class="mb-2"><strong>{{ $lesson->lesson_title }} </strong>
                            <span class="small"><a
                                    href="{{ route('teacher.edit-lessons', $lesson->id) }}">{{ trans('learning::local.edit') }}
                                </a>
                            </span>
                        </h3>
                ...
                <div class="card-content collapse show">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <a href="#" onclick="attachments()" class="btn btn-success btn-sm"><i
                                    class="la la-paperclip"></i> {{ trans('learning::local.add_attachments') }}</a>
                            <a href="#" onclick="attachmentDestroy()" class="btn btn-danger btn-sm"><i
                                    class="la la-trash"></i> {{ trans('admin.delete') }}</a>
                        </div>
                        {{-- attachments --}}
                                ...
                                <ul style="list-style: none">
                                    @foreach ($lesson->files as $file)
                                        <li>
                                            ...
                                            <a target="_blank"
                                                href="{{ asset('images/lesson_attachments/' . $file->file_name) }}">{{ $file->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </form>
                        ...
                        <hr>
                        {{-- playlist --}}
                        <h5><strong>{{ trans('learning::local.playlist') }} :</strong> <a
                                href="{{ route('teacher.show-lessons', $lesson->playlist_id) }}">{{ $lesson->playlist->playlist_name }}</a>
                        </h5>
                        <ol>
                            @foreach ($lessons as $lesson_playlist)
                                @if ($lesson_playlist->id != $lesson->id)
                                    <li>
                                        <a
                                            href="{{ route('teacher.view-lesson', ['id' => $lesson_playlist->id, 'playlist_id' => $lesson_playlist->playlist_id]) }}">
                        ...
                        <hr>
                        <div class="form-group">
                            <h6><strong>{{ trans('learning::local.subject_type') }} :
                                </strong>{{ session('lang') == 'ar' ? $lesson->subject->ar_name : $lesson->subject->en_name }}
                            </h6>
                            <h6><strong>{{ trans('learning::local.created_by') }} :
                                </strong>{{ session('lang') == 'ar' ? $lesson->admin->ar_name : $lesson->admin->name }}</h6>
                            <h6><strong>{{ trans('learning::local.created_at') }} :
                                </strong>{{ $lesson->created_at->diffForHumans() }}</h6>
                            <h6><strong>{{ trans('learning::local.last_updated') }} :</strong>
                                :{{ $lesson->updated_at->diffForHumans() }}</h6>
                        </div>
                        ...
                        @foreach ($lesson->exams as $exam)
                            <div class="mb-1 badge badge-info">
                                <span><a target="_blank"
                                        href="{{ route('teacher.preview-exam', $exam->id) }}">{{ $exam->exam_name }}</a></span>
                        ...
                        <div class="col-md-12">
                            <form action="{{ route('teacher.approval') }}" method="POST">
                                ...
                                <select name="approval" class="form-control" onchange="this.form.submit()">
                                    <option {{ $lesson->approval == 'pending' ? 'selected' : '' }} value="pending">
                                        {{ trans('learning::local.pending') }}
                                    </option>
                                    <option {{ $lesson->approval == 'accepted' ? 'selected' : '' }} value="accepted">
                                        {{ trans('learning::local.accepted') }}
                                    </option>
                                </select>
                            </form>
                        </div>
                ...
                swal({
                    ...
                    function() {
                        $.ajax({
                            ...
                            .done(function(data) {
                                if (data.status == true) {
                                    swal("{{ trans('msg.delete') }}", "{{ trans('msg.delete_successfully') }}",
                                        "success");
                                } else {
