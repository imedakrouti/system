...
<h4 class="center">{{ trans('student::local.daily_requests') }} 
...
<table>
    <thead>
        <tr>
            ...
            <th>{{ trans('student::local.leave_date') }}</th>
            <th>{{ trans('student::local.leave_time') }}</th>
        </tr>
    </thead>
    <tbody>
        {{$n=1}}
        @foreach ($daily_requests as $request)
            <tr>
                <td>{{$n}}</td>
                ...
                <td>{{ date_format($request->created_at,"Y/m/d")}}</td>
