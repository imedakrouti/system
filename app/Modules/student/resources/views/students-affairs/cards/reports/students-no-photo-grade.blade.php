...
<htmlpageheader name="page-header"> 
    <h3 class="center">{{ trans('student::local.students_no_photo') }} [{{$division_name}} - {{$grade_name}}] </h3>
</htmlpageheader>
<table>
    <thead>
        <tr>
            ...
            <th>{{ trans('student::local.class_room') }}</th>
        </tr>
    </thead>
    <tbody>
        {{$n = 1}}
        @foreach ($students as $student)
            <tr>
                ...
                <td>
                    @if (session('lang') == 'ar')
                        {{$student->ar_name_classroom}}
                    @else
                        {{$student->en_name_classroom}}
                    @endif
                </td>
            </tr>
            {{$n++}}
